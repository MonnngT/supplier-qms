...
for dim in dimensions:
    ...
    c1.write(f"**{dim}**")
    mode = c2.selectbox("模式", ["输入数值", "实配 (Pass)"], key=f"mode_{dim}_{st.session_state.form_version}", label_visibility="collapsed")
    if mode == "输入数值":
        val = c3.text_input("数值", key=f"val_{dim}_{st.session_state.form_version}", label_visibility="collapsed", placeholder="输入...")
    else:
        c3.text_input("实配", value="已实配 / OK", disabled=True, key=f"dis_{dim}_{st.session_state.form_version}", label_visibility="collapsed")
        val = "实配"
...
if st.button("📤 提交数据到系统", type="primary", use_container_width=True):
    ...
    if empty_fields:
        ...
        new_row = {"记录生成时间": record_time, "测量时间": full_measure_datetime, "PartName": selected_part, **input_results}
...
try:
    ...
    if not df_history.empty:
        ...
        if selected_history:
            ...
            his_part_name, his_measure_time = row_data["PartName"], row_data["测量时间"]
